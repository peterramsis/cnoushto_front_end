<template>
  <div class="container">
    <section class="register-card">
      <form action="" @submit="register">
        <section class="row">
          <h3 class="text-center text-white">التسجيل</h3>
          <hr />
          <div
            class="col-12 col-md-6 col-lg d-flex justify-content-center align-items-center"
          >
            <section class="photo-picture">
              <img :src="photoUrl" alt="" srcset="" class="img-fluid" />
            </section>
          </div>
          <div class="col-12 col-md-6 col-lg">
            <section class="">
              <label for="" class="form-label text-white">الصورة الشخصية</label>
              <input
                type="file"
                class="form-control"
                placeholder=""
                @change="onImageChange"
              />
              <span class="text-danger" v-if="store.errors.image">  {{ store.errors.image[0] }}  </span>
            </section>

            <section class="">
              <label for="" class="form-label text-white">الاسم</label>
              <input
                type="text"
                class="form-control"
                placeholder=""
                v-model="store.name"
               
              />
              <span class="text-danger" v-if="store.errors.name">  {{ store.errors.name[0] }}  </span>
            </section>

            <section class="">
              <label for="" class="form-label text-white">الموبايل</label>
              <input
                type="text"
                class="form-control"
                placeholder=""
                v-model="store.phone"
              />
              <span class="text-danger" v-if="store.errors.phone">  {{ store.errors.phone[0] }}  </span>
            </section>

            <section class="">
              <label for="" class="form-label text-white">تاريخ الميلاد</label>
              <input
                type="date"
                class="form-control"
                placeholder=""
                v-model="store.birthday"
               
              />
              <span class="text-danger" v-if="store.errors.birthday">  {{ store.errors.birthday[0] }}  </span>
            </section>

            <section class="">
              <label for="" class="form-label text-white">البريد الالكترونى</label>
              <input
                type="text"
                class="form-control"
                placeholder=""
                v-model="store.email"
               
              />
              <span class="text-danger" v-if="store.errors.email">  {{ store.errors.email[0] }}  </span>
            </section>

            <section class="">
              <label for="" class="form-label text-white"> كلمة المرور </label>
              <input
                type="password"
                class="form-control"
                placeholder=""
                v-model="store.password"
               
              />
              <span class="text-danger" v-if="store.errors.password">  {{ store.errors.password[0] }}  </span>
            </section>
            <section class="">
                <label for="" class="form-label text-white">اعادة كلمة المرور </label>
                <input
                  type="password"
                  class="form-control"
                  placeholder=""
                  v-model="store.password_confirmation"
                 @keyup="checkPassword"
                />
                <span class="text-danger" v-if="disabled"> كلمة المرور غير متطابقة </span>
            </section>



            <section class="">
              <label for="" class="form-label text-white"> الكنيسة </label>
              <input
                type="text"
                class="form-control"
                placeholder=""
                v-model="store.church"
               
              />
              <span class="text-danger" v-if="store.errors.church">  {{ store.errors.church[0] }}  </span>
            </section>

            <section class="">
              <label for="" class="form-label text-white" > الجنس </label>
              <select name="" class="form-control" v-model="store.gender">
                <option value="">--اختار--</option>
                <option value="ذكر">ذكر</option>
                <option value="انثى">انثى</option>
              </select>
              <span class="text-danger" v-if="store.errors.gender">  {{ store.errors.gender[0] }}  </span>
            </section>

            <section class="d-flex justify-content-center">
              <button-app
                class="mt-4"
                title="تسجيل"
                backgroundColor="#4F0E0E"
                hoverColor="#FFF1F1"
                colorText="#4F0E0E"
                :disabled="disabled"
              ></button-app>
            </section>
          </div>
        </section>
      </form>
    </section>
  </div>
</template>

<style lang="scss" scoped>

.register-card {
  .photo-picture {
    img {
      height: 300px;
      margin: 10px auto;
      /* width: 100%; */
      object-fit: cover;
      display: flex;
      border-radius: 51%;
    }
  }
  form {
    font-family: "Cairo", sans-serif;

    background-color: #dfbda3;
    padding: 10px 20px;
    @include border-radius(10px);

    input[type="text"], input[type="password"] {
      @include border-radius(12px);
    }
  }
}
</style>

<script>
import { ref } from 'vue';
import { useAuthStore } from '@/stores/modules/auth';
export default {
  name: "register",
  setup() {
    const photo = ref(null);
    const disabled = ref(false);
    const data = new FormData();

    const store = useAuthStore();
    const photoUrl = ref(
      "data:image/png;base64,"
    );

    function onImageChange(e) {
      let files = e.target.files || e.dataTransfer.files;
      if (!files.length) return;

      const file = e.target.files[0];

      data.append("image",file);


      const reader = new FileReader();
      reader.onload = (e) => {
        photo.value = file;
        photoUrl.value = e.target.result;
      };
      reader.readAsDataURL(file);
    }

    function checkPassword(){

        console.log("click");
        if(store.password_confirmation != store.password && store.password_confirmation != "" && store.password != ""){
           disabled.value = true;
        }else{
            disabled.value = false;
        }
    }

    function register(e){
        e.preventDefault();
        console.log("click");

        data.append("name" , store.name);
        data.append("email", store.email);
        data.append("password", store.password);
        data.append("phone", store.phone);
        data.append("church", store.church);
        data.append("gender", store.gender);
        data.append("birthday", store.birthday);
       store.register(data);

      // store.test();
    }


    return {
      onImageChange,
      photo,
      disabled,
      photoUrl,
      store,
      checkPassword,
      disabled,
      register
    };
  },
};
</script>
